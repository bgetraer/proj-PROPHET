...
if perform(org,'Bathymetry'), % {{{
	...
	% Threshold the bathymetry to the mesh
   B_final = mit.mesh.zp(k_ind) - hFacDim;

	% Save Bathymetry data
	mit.geometry=struct();
	mit.geometry.bathy   = B_final; % the final processed bathymetry (m)
...
if perform(org,'RuntimeOptionsCoupled') % {{{
	...
	% The data parameters are set during
	%	during the run
	disp(' - Setting timestepping options');
	...
	mit.timestepping.coupled_endtime  = 600*coupling_dT;                                             % the end model time that we are aiming for (s)
	mit.timestepping.deltaT_coupled   = coupling_dT;                                               % coupling time step (s)
	mit.timestepping.nsteps           = mit.timestepping.coupled_endtime/mit.timestepping.deltaT_coupled; % number of coupled time steps to take
   ...
   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	runcoupleddir = fullfile(expdir,sprintf('runcoupledRUN02_dt%03i_ct%07i',mit_dT,coupling_dT)); % run directory
	% rename previous run directory and create new one
	...
	source_fnames={'draft.bin',dataobcsfile,'bathy.bin'};
	destination_fname={['draft.' suffix '.bin'], 'data.obcs','bathy_ref.bin'};
   disp(['   copying ' num2str(numel(source_fnames)) ' files from']);
   disp(['       ' inputdir ' to']);
   disp(['       ' runcoupleddir]);
   for i=1:numel(source_fnames)
...
if perform(org,'RunCoupled') % {{{
	rundir = fullfile(expdir,sprintf('runcoupledRUN02_dt%03i_ct%07i',mit_dT,coupling_dT)); % run directory
...
if perform(org,'RunPickup') % {{{
	...
	fprintf('Experiment: %s Year: %i\n',experiment.name,round(modeltime_pickup/mit.timestepping.y2s+2010))
   ...
   mit.timestepping.nsteps           = (291*360 - modeltime_pickup/3600/24)/15;   % number of coupled time steps to take
...
% Run beyond 2100
if perform(org,'Forcings2100') % {{{
	rundir = fullfile(expdir,sprintf('runcoupledRUN02_dt%03i_ct%07i',mit_dT,coupling_dT)); % run directory

	years = reshape(2101:2300,10,20); % column: decade; row: index year 1--10
	ref_year = 2091:2100; % reference year
	assert(numel(ref_year)==size(years,1));
	for i=1:size(years,1)
		file_suffix = sprintf('%s_%i',experiment.name,ref_year(i));
		filenames={dir(fullfile(rundir,['*' file_suffix])).name};
		for j=1:numel(filenames)
			file_prefix = extractBefore(filenames{j},file_suffix);
			for k=1:size(years,2)
				out_suffix = sprintf('%s_%i',experiment.name,years(i,k));
				out_file = [file_prefix out_suffix];

				command = sprintf('ln %s %s',fullfile(rundir,filenames{j}),fullfile(rundir,out_file));
				fprintf(' %s -> %s\n',filenames{j},out_file);
				system(command);
			end
		end
	end
